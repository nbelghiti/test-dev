...
export class ErrorMiddleware {
	public static handleError(err: string | Error | ICustomError | null | undefined, req: IRequest, res: IResponse, next: INext): IResponse | void {
		...
		logger.error(`${errResponse.name}\n"${errResponse.message}" at ${errResponse.timestamp}\n${JSON.stringify(errResponse.stack, null, 2)}`);

		return res.status((err as ICustomError).status).json(errResponse);
	}
}
